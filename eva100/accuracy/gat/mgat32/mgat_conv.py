...
class MGATFunction(torch.autograd.Function):
    ...
    def forward(ctx, X_prime, a0, a1, inputInfo):
        ...
        att= MagicsphereGAT.forward_tf32(X_prime.size(1), inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, inputInfo.values, X_prime, a0, a1,inputInfo.max)
        #过滤掉0元素
    ...
    def backward(ctx, att_grad):
        ...
        '''
        #att_grad求a0梯度
        a0_tmp= MagicsphereGCN.forward_tf32(inputInfo.row_pointers, inputInfo.column_index, att_grad, X_prime, inputInfo.num_nodes, X_prime.size(1), inputInfo.num_nodes_ori)
        #按列求和，求出a0
        a0_grad = torch.mm(inputInfo.ones.t(), a0_tmp)
        #att_grad求a1梯度
        a1_tmp= MagicsphereGCN.forward_tf32(inputInfo.row_pointers, inputInfo.column_index, att_grad, inputInfo.ones, inputInfo.num_nodes, inputInfo.ones.size(1), inputInfo.num_nodes_ori)
...
class MGATSpmm(torch.autograd.Function):
    ...
    def forward(ctx, att, X_prime, inputInfo):
        ...
        X_prime = MagicsphereGCN.forward_tf32(inputInfo.row_pointers, inputInfo.column_index, att, X_prime, inputInfo.num_nodes, X_prime.size(1), inputInfo.num_nodes_ori)
        return X_prime
    ...
    def backward(ctx, X_prime_grad):
        ...
        inputInfo = ctx.inputInfo
        att = ctx.att
        att_trans = MagicsphereGAT.trans_gat_tf32(inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, att, inputInfo.values_templete, inputInfo.max)
        #求X_prime_grad
        d_X_prime= MagicsphereGCN.forward_tf32(inputInfo.row_pointers, inputInfo.column_index, att_trans, X_prime_grad, inputInfo.num_nodes, X_prime_grad.size(1), inputInfo.num_nodes_ori)
        #根据X_prime，通过SDDMM反向传播求att_grad梯度
        d_att = MagicsphereGAT.forward_gen_tf32(X_prime.size(1), inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, inputInfo.values, X_prime_grad, X_prime, inputInfo.max)
        #过滤掉0元素
