...
class GCN_dataset(torch.nn.Module):
    ...
    def init_edges(self, density, partSize,  window, wide, kernel):
        ...
        self.num_nodes_ori  = self.graph['num_nodes_src']
...
class GCN_dataset_cuda_v2(torch.nn.Module):
    ...
    def init_edges(self, density, partSize, shortSize):
        ...
        self.dst_li=self.graph['dst_li']
        self.num_nodes_ori  = self.graph['num_nodes_src']
        self.num_nodes_dst = self.graph['num_nodes_src']
        self.num_nodes = self.graph['num_nodes_src']
        self.num_edges = len(self.src_li)
        self.edge_index = torch.from_numpy(np.stack([self.src_li, self.dst_li]))
        val = [1] * self.num_edges
        scipy_coo = coo_matrix((val, self.edge_index), shape=(self.num_nodes, self.num_nodes_dst))
    ...
    #     self.c_atomicTensor_=self.c_atomicTensor.to(device)
    #     self.c_colTensor_=self.c_colTensor.to(device)
    #     self.c_valueTensor_=self.c_valueTensor.half().to(device)
    ...
    #     self.c_valueTensor_short_=self.c_valueTensor_short.half().to(device)
      
    #     self.x =  self.x.half().to(device)
    #     return self
...
class GCN_dataset_tcu_v2(torch.nn.Module):
    ...
    #     self.t_blockNew_offsetTensor_=self.t_blockNew_offsetTensor.to(device)
    #     self.t_columnTensor_=self.t_columnTensor.to(device)
    #     self.t_valueTensor_=self.t_valueTensor.half().to(device)
    #     self.t_window_rowTensor_=self.t_window_rowTensor.to(device)
...
class GCN_dataset_tcu(torch.nn.Module):
    ...
    def init_edges(self, density, partSize,  window, wide):
        ...
        self.t_valueTensor, \
        self.t_rowTensor, \
        self.t_colTensor = Libra5Block.block_csr_tcu(self.row_pointers, self.column_index,self.degrees, partSize,  window, wide)
...
class GCN_dataset_tcu_binary(torch.nn.Module):
    ...
    def init_edges(self, density, partSize,  window, wide):
        ...
        self.t_binaryTensor = self.t_binaryTensor
        self.t_valueTensor = self.t_valueTensor.half()
        # self.boundary =  self.t_window_rowTensor.shape[0]
    ...
    def init_embedding(self):
        ...
        Called from __init__.
        '''
        self.x = torch.randn(self.num_nodes_dst, self.num_features).half()
...
class GCN_dataset_tcu_cuda_tf32(torch.nn.Module):
    ...
    def init_edges(self, density, partsize_t, partsize_c, shortsize,  window, wide):
        ...
        self.c_atomicTensor_short, \
        self.c_colTensor_short, \
        self.c_valueTensor_short , self.duration= Libra5Block.block_tf32_tcu_cuda_binary(self.row_pointers, self.column_index, self.degrees, partsize_t, partsize_c, shortsize, density, window, wide)
        
        cuda_num = self.c_valueTensor.shape[0] + self.c_valueTensor_short.shape[0]
        tcu_num_percen = round(((self.num_edges-cuda_num) / self.num_edges ),4) * 100
        print(tcu_num_percen)
