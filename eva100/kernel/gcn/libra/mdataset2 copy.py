...
class GCN_dataset(torch.nn.Module):
    ...
    def init_edges(self, density, partSize):
        ...
        self.t_row_offsetTensor, self.t_colTensor, self.t_valueTensor,  self.t_rowTensor,  self.t_c_partTensor,  self.t_c_row_offsetTensor,  self.t_c_rowTensor,  self.t_c_atomicTensor,  self.t_c_colTensor,  self.t_c_valueTensor, self.c_row_offsetTensor, self.c_rowTensor, self.c_atomicTensor, self.c_colTensor, self.c_valueTensor= Libra5Block.block_16_4_balance(self.row_pointers, self.column_index,self.degrees, density, partSize)
        ...
        # self.t_row_offsetTensor, self.t_colTensor, self.t_valueTensor,  self.t_rowTensor, self.c_partTensor, self.c_row_offsetTensor, self.c_rowTensor, self.c_atomicTensor, self.c_colTensor, self.c_valueTensor= Libra5Block.block_8_4_split_v2(self.row_pointers, self.column_index,self.degrees, density, partSize)

        self.boundary =  self.t_rowTensor.shape[0]
        self.parts = self.c_row_offsetTensor.shape[0]
        print("boundary, parts: " + str(self.boundary) + ", " + str(self.parts))
    ...
    def to(self, device):
        ...
        self.t_c_row_offsetTensor_=self.t_c_row_offsetTensor.to(device)
        self.t_c_colTensor_=self.t_c_colTensor.to(device)
        self.t_c_valueTensor_=self.t_c_valueTensor.to(device)
        self.t_c_partTensor_=self.t_c_partTensor.to(device)
        self.t_c_rowTensor_=self.t_c_rowTensor.to(device)
        self.t_c_atomicTensor_=self.t_c_atomicTensor.to(device)
