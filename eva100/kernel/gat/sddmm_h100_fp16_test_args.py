...
if __name__ == "__main__":
    ...
    #用于存储结果
    file_name = '/home/shijinliang/module/Libra/res/h100/sddmm/fp16/h100_sddmm_fp16_result_' + str(dimN) + '.csv'
    head = ['dataSet', 'num_nodes', 'num_edges', '4', '8', '14', '18', '22', '30', '38','50', 'density', 'speedup']
    ...
    dataset =[   'LastFM','twitter', 'MOOC',
             'IGB_small', 'reddit',  'yeast']
    for data in dataset:
        graph = np.load('./dgl_dataset/mythroughput/' + data +'.npz')
        src_li=graph['src_li']
        num_nodes_ori  = graph['num_nodes']
        ...
        # #执行纯tcu without part
        # spmm_tcu_csr = libra_csr_test_tcu(data, hidden, epoches, 1, partsize_t, data_path, window, wide)
        
        # #执行纯tcu without part_binary
        # spmm_tcu_binary = libra_binary_test_tcu(data, hidden, epoches, 1, partsize_t, data_path, window, wide)
        

        # #执行纯cuda with part    
        # spmm_cuda = libra_csr_test_cuda(data, hidden, epoches, 1, partsize_c, data_path, True, shortsize)
        # # spmm_cuda_navie = libra_csr_test_cuda_v2(data, hidden, epoches, 1, partsize_c, data_path, True, shortsize)
        # # type = 0
        ...
        for dense in density:
            spmm = libra_csr_test_tcu_cuda(data, hidden, epoches, dense,partsize_t, partsize_c, data_path, window, wide, type)
            res_temp.append(spmm)
            print()
            
        sub_list = res_temp[3:11]
    ...
    # file_name_concat = '/home/shijinliang/module/Libra/res/h100/sddmm/fp16/h100_sddmm_fp16_result_concat_' + str(dimN) + '.csv'
