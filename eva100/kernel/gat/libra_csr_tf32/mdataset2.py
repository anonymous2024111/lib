...
class GCN_dataset_tcu_v2(torch.nn.Module):
    ...
    def init_edges(self, density, partSize,  window, wide):
        ...
        self.t_atomicTensor, \
        self.t_rowTensor, \
        self.t_colTensor = Libra5Block.block_csr_tcu_part(self.row_pointers, self.column_index,self.degrees, partSize,  window, wide)
...
class GCN_dataset_tcu1(torch.nn.Module):
    """
    data loading for more graphs
    """
    def __init__(self, data, dimN, density,partsize, data_path,  window, wide):
        super(GCN_dataset_tcu1, self).__init__()
    ...
    def init_edges(self, density, partSize,  window, wide):
        ...
        self.t_window_rowTensor, \
        self.t_atomicTensor, \
        self.t_rowTensor = Libra5Block.block_csr_binary_sddmm_tcu_part_c(self.row_pointers, self.column_index, partSize,  window, wide)
...
class GCN_dataset_cuda_tcu_binary(torch.nn.Module):
    """
    data loading for more graphs
    """
    def __init__(self, data, dimN, density, partsize_t, partsize_c, data_path,  window, wide):
        super(GCN_dataset_cuda_tcu_binary, self).__init__()

        self.graph = np.load('./dgl_dataset/' + data_path + '/' + data +'.npz')
        self.num_features = dimN
        self.init_edges(density,  partsize_t, partsize_c, window, wide)
        self.init_embedding()

        
        
    def init_edges(self, density,  partsize_t, partsize_c,  window, wide):
        # loading from a .npz graph file
        self.num_nodes_ori =  self.graph['num_nodes_src']-0
        self.num_nodes_dst =  self.graph['num_nodes_dst']-0
        self.num_nodes = self.num_nodes_ori
        ...
        self.c_row_offsetTensor, \
        self.c_rowTensor, \
        self.c_colTensor = Libra5Block.block_tf32_tcu_cuda_binary_sddmm(self.row_pointers, self.column_index,  partsize_t, partsize_c, density, window, wide)
