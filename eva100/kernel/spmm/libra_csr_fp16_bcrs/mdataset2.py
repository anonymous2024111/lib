...
class GCN_dataset_tcu_v2(torch.nn.Module):
    ...
    def init_edges(self, density, partSize,  window, wide):
        ...
        self.t_valueTensor, \
        self.t_window_rowTensor, \
        self.t_atomicTensor = Libra6SpMM.block_fp16_tcu_bcrs(self.row_pointers, self.column_index,self.degrees, partSize,  window, wide)

        print(self.t_columnTensor.shape)
...
class GCN_dataset_tcu_cuda_tf32(torch.nn.Module):
    ...
    def init_edges(self, density, partsize_t, partsize_c, shortsize,  window, wide):
        ...
        self.duration = Libra6SpMM.block_fp16_tcu_cuda_bcrs(self.row_pointers, self.column_index, self.degrees, partsize_t, partsize_c, shortsize, density, window, wide)


        
        tcu_part = self.t_valueTensor.size(0)
        cuda_part = self.num_edges - tcu_part
        print("tcu: " + str(tcu_part) + ";   cuda: " + str(cuda_part))
