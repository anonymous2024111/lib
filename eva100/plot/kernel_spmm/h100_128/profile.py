...
df1_tf32 = df1[['libra_fp16','dataSet','spmm_tcu']]
...
for index, row in df_res.iterrows():
    if row['tcgnn'] != 1000000000:
        tcgnn.append(round((row['tcgnn']/row['libra_fp16']),4))
        total_tcgnn +=1
    ...
    dtc.append(round((row['dtc']/row['libra_fp16']),4))
    flash.append(round((row['spmm_tcu']/row['libra_fp16']),4))
    total+=1
    if row['rode'] > row['libra_fp16']:
        libra_road+=1
#统计256的数据
...
#     sputnik.append(round((row['Sputnik_time']/row['libra_fp16']),4))
#     cusparse.append(round((row['cuSPARSE_time']/row['libra_fp16']),4))
#     rode.append(round((row['ours_time']/row['libra_fp16']),4))
#     total+=1
print()
# 统计分布
# 1. TC-GNN
