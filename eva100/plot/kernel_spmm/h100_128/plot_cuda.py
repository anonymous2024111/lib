...
for index, row in df_res.iterrows():
    ...
    if (round((compute/row['libra_fp16'])*1e-6,4))>11000:
        libra_G_fp16.append(11000)
    else:
...
# plt.figure(figsize=(6, 3.5))  # 设置宽度为 10，高度为 6
fig, ax = plt.subplots(figsize=(7, 2.5))
#vs 
# sns.scatterplot(x=num_edges_str, y=sputnik_G_avg, label='Sputnik', color='orange', edgecolor='none')
# sns.scatterplot(x=num_edges_str, y=cusparse_G_avg, label='cuSPARSE', color='grey', edgecolor='none')
# sns.scatterplot(x=num_edges_str, y=rode_G_avg, label='RoDe', color='red', edgecolor='none')
# sns.scatterplot(x=num_edges_str, y=tcgnn_avg, label='TC-GNN',  color='lightgreen', edgecolor='none')
# sns.scatterplot(x=num_edges_str, y=dtc_G_avg, label='DTC-SpMM',  color='plum', edgecolor='none')
# sns.scatterplot(x=num_edges_str, y=libra_G_fp16_avg, label='Libra-TF32', color='coral', edgecolor='none')
num_edges_str = np.log10(num_edges_str)
ax.scatter(num_edges_str, cusparse_G_avg, color='khaki', s=12)
ax.scatter(num_edges_str, sputnik_G_avg, color='pink', s=12)
ax.scatter(num_edges_str, rode_G_avg, color='tomato', s=12)
ax.scatter(num_edges_str, libra_G_fp16_avg, color='blue', s=12)
...
# 显示图形
plt.savefig('/home/shijinliang/module/Libra-sc25/eva100/plot/kernel_spmm/h100_128/h100_spmm_fp16_128_cuda.png', dpi=800)
# 清空图形
plt.clf()
