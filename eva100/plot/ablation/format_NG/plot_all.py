...
df1 = pd.read_csv('/home/shijinliang/module/Libra/eva100/plot/ablation/format_NG/result/fp16_test_all_v2.csv')
...
for index, row in df_res.iterrows():
    ...
    if row['spmm_tcu'] < row['spmm_tcu_binary'] and row['spmm_tcu_binary'] < row['spmm_tcu_sgt'] :
        ...
        if row['spmm_tcu'] < row['spmm_tcu_binary']:
            NG_effctive+=1
            NG_speedup.append(round((row['spmm_tcu_sgt']/row['spmm_tcu']),4))
        ...
        if row['spmm_tcu_binary'] < row['spmm_tcu_dtc'] :
            ...
            format_dtc_speedup.append(round((row['spmm_tcu_sgt']/row['spmm_tcu_dtc']),4))
...
tcu_dtc_G = [tcu_bianry_G[i] for i in sorted_indices]
...
b_percen = round((b/(a+b+c)*100), 2)
print("<1.5 ",a_percen, '%')
print("<2 ", b_percen,  '%')
print(">=2 ", (100-a_percen-b_percen),  '%')
