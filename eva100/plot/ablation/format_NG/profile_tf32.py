...
for index, row in df_res.iterrows():
    ...
    dtc_G.append(round((compute/row['spmm_tcu_dtc'])*1e-6,4))
    ...
    if row['spmm_tcu_binary'] < row['spmm_tcu_dtc'] :
        ...
        format_dtc_speedup.append(round((row['spmm_tcu_sgt']/row['spmm_tcu_binary']),4))
...
print("DTC Format 有效矩阵个数: " + str(format_dtc_effctive) + " / " + str(total) + '; 平均加速比: ' + str(mean(format_dtc_speedup)) + '; 最高加速比: ' + str(max(format_dtc_speedup)) )
