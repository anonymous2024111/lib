...
class GCN_dataset_tcu_stream(torch.nn.Module):
    """
    data loading for more graphs
    """
    def __init__(self, data, dimN, density,partsize, data_path,  window, wide):
        super(GCN_dataset_tcu_stream, self).__init__()
    ...
    def init_edges(self, density, partSize,  window, wide):
        ...
        self.t_valueTensor, \
        self.t_rowTensor,\
        self.t_rowNew_offsetTensor1, \
        self.t_blockNew_offsetTensor1, \
        self.t_columnTensor1, \
        self.t_valueTensor1, \
        self.t_rowTensor1= Libra5Block.block_csr_tcu_stream(self.row_pointers, self.column_index,self.degrees, partSize,  window, wide)
...
class GCN_dataset_tcu_sgt(torch.nn.Module):
    ...
    def __init__(self, data, dimN, density,partsize, data_path,  window, wide):
        ...
        self.graph = np.load('/home/shijinliang/module/Libra/dgl_dataset/' + data_path + '/' + data +'.npz')
        self.num_features = dimN
        self.init_edges(density, partsize, window, wide)
        self.init_embedding()
    ...
    def init_edges(self, density, partSize,  window, wide):
        ...
        self.t_windowkNew_offsetTensor, \
        self.t_blockNew_offsetTensor, \
        self.t_columnTensor, \
        self.t_valueTensor, \
        self.t_rowTensor = Libra5Block.block_csr_tcu(self.row_pointers, self.column_index,self.degrees, partSize,  window, wide)
        self.t_rowTensor =  self.t_rowTensor.to(torch.int8)
        # self.boundary =  self.t_window_rowTensor.shape[0]
