...
for index, row in df_res.iterrows():
    ...
    if (round((compute/row['libra'])*1e-6,4))>8000:
        continue
...
# indices_to_remove = [i for i, x in enumerate(libra_G_avg) if x > 7000]
# libra_G_avg = [x for i, x in enumerate(libra_G_avg) if i not in indices_to_remove]
# tcgnn_G_avg = [x for i, x in enumerate(tcgnn_G_avg) if i not in indices_to_remove]
...
# flash_G_avg = [x * 0.85 for x in flash_G_avg]
...
# ax.legend()
# 显示图形
plt.savefig('/home/shijinliang/module/Libra-sc25/eva100/plot/kernel_sddmm/h100_32/h100_sddmm_tf32_32_tcu.png', dpi=800)
# 清空图形
plt.clf()
