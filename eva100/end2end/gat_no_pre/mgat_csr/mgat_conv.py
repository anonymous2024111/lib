...
class MGATFunction(torch.autograd.Function):
    ...
    def forward(ctx, X_prime, a0, a1, inputInfo):
        ...
        # ctx.a0 = a0
        # ctx.a1 = a1
        output_a0, output_a1=MagicsphereGAT_cmake.fp16_a_feature(X_prime.size(1), inputInfo.num_nodes, X_prime, a0, a1)
        att = MagicsphereGAT_cmake.fp16_csr_v2(X_prime.size(1), inputInfo.num_nodes, inputInfo.row_pointers1, inputInfo.column_index1, output_a0, output_a1,inputInfo.num_edges)
        # att = MagicsphereGAT_cmake.fp16_csr(X_prime.size(1), inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, inputInfo.values, output_a0, output_a1, inputInfo.max, inputInfo.num_edges)
    ...
    def backward(ctx, att_grad):
        ...
        '''
        #att_grad求a0梯度
        a0_tmp= MagicsphereGCN_cmake.forward_v2_csr(inputInfo.row_pointers, inputInfo.column_index, inputInfo.values_templete, att_grad, X_prime, inputInfo.num_nodes, X_prime.size(1), inputInfo.num_nodes_ori)
        #按列求和，求出a0
        a0_grad = torch.mm(inputInfo.ones.t(), a0_tmp)
        
        #att_grad求a1梯度
        a1_tmp= MagicsphereGCN_cmake.forward_v2_csr(inputInfo.row_pointers, inputInfo.column_index, inputInfo.values_templete, att_grad, inputInfo.ones, inputInfo.num_nodes, inputInfo.ones.size(1), inputInfo.num_nodes_ori)
...
class MGATSpmm(torch.autograd.Function):
    ...
    def forward(ctx, att, X_prime, inputInfo):
        ...
        X_prime = MagicsphereGCN_cmake.forward_v2_csr(inputInfo.row_pointers, inputInfo.column_index, inputInfo.values_templete, att, X_prime, inputInfo.num_nodes, X_prime.size(1), inputInfo.num_nodes_ori)
    ...
    def backward(ctx, X_prime_grad):
        ...
        d_X_prime= MagicsphereGCN_cmake.forward_v2_csr(inputInfo.row_pointers, inputInfo.column_index, inputInfo.values_templete_t, att, X_prime_grad, inputInfo.num_nodes, X_prime_grad.size(1), inputInfo.num_nodes_ori)
        
        #根据X_prime，通过SDDMM反向传播求att_grad梯度
        d_att = MagicsphereGAT_cmake.fp16_sddmm_csr(X_prime.size(1), inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, inputInfo.values, X_prime_grad, X_prime, inputInfo.max, inputInfo.num_edges)
...
class GATConv(torch.nn.Module):
    ...
    def forward(self, X, inputInfo):
        ...
        att = MGATFunction.apply(X_prime, self.a0.half(), self.a1.half(), inputInfo)
        #leakrelu
