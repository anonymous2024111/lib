...
# DGL
with open('./eva100/end2end/gcn_no_pre/result_small/dgl.csv', 'r') as file:
...
# Advisor
with open('./eva100/end2end/gcn_no_pre/result_small/advisor.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
...
# PYG
with open('./eva100/end2end/gcn_no_pre/result_small/pyg.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
# TC-GNN
with open('./eva100/end2end/gcn_no_pre/result_small/tcgnn.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
# MGCNtf32
with open('./eva100/end2end/gcn_no_pre/result_small/mgcn32.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
# MGCNfp16
with open('./eva100/end2end/gcn_no_pre/result_small/mgcn16.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
g = sns.boxplot(x='layer-hidden', y='speedup', hue='baseline', data=df, 
                palette=mycolor, linewidth=1, legend=True, gap=0.1, width=0.8)
...
# 显示图形
plt.savefig('./eva100/end2end/gcn_no_pre/result_small/gcn_kernel_small.png', dpi=800)
# 清空图形
plt.clf()
