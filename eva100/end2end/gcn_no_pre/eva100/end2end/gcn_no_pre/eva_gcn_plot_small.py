...
# DGL
with open('./eva100/end2end/gcn_no_pre/result_small/dgl-v2.csv', 'r') as file:
...
# Advisor
with open('./eva100/end2end/gcn_no_pre/result_small/advisor-v2.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
...
#PYG
with open('./eva100/end2end/gcn_no_pre/result_small/pyg-v2.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
#TC-GNN
with open('./eva100/end2end/gcn_no_pre/result_small/tcgnn-v2.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
# MGCNtf32
with open('./eva100/end2end/gcn_no_pre/result_small/mgcn32-v2.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
# MGCNfp16
with open('./eva100/end2end/gcn_no_pre/result_small/mgcn16-v2.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for cur, row in enumerate(reader, start=0):
        speedup['layer-hidden'].append(row[1])
...
# 显示图形
plt.savefig('./eva100/end2end/gcn_no_pre/result_small/gcn_kernel_100.png', dpi=800)
# 清空图形
plt.clf()
