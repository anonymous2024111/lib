...
middataset = ['blog', 'amazon']
midhidden = ['256', '512', '1024']
hugedataset = ['reddit', 'IGB_medium','AmazonProducts']
hugehidden = ['128', '256', '512']
...
dim = dict()
cur_row=0
with open('./eva100/abalation/gcn/tf32-H100.csv', 'r') as file:
    reader = csv.reader(file, delimiter=',')
    next(reader)  # 跳过第一行
    for row in reader:
        if row[0] in middataset and row[1] in midhidden:
            for i in range(4):
                result['dataset'].append(row[0]+'-'+str(row[1]))
                result['time'].append(float(row[2+i]))
                result['type'].append(i)
        if row[0] in hugedataset and row[1] in hugehidden:
            for i in range(4):
                result['dataset'].append(row[0]+'-'+str(row[1]))
...
plt.savefig('./eva100/abalation/gcn/tf32-H100.png', dpi=800)
