...
if __name__ == "__main__":
    ...
    res['baseline'] = []
    res['acc'] = []   
    with open('./eva100/accuracy/gat/result.csv', 'r') as file:
    ...
    g = sns.barplot(x='data', y='acc', hue='baseline', data=df, 
                    palette='Blues_d', linewidth=1, legend=True, gap=0.1, width=0.8)

    plt.ylim(40, 100)  # 将 Y 轴下限设置为 40
    sns.despine(left=True, right=True, top=True)
    g.set_ylabel('')
    g.set_xlabel('')
    plt.savefig('./eva100/accuracy/gcn/gat.png', dpi=800)
    print("success")
