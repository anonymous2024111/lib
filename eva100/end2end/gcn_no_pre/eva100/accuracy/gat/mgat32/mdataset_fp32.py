...
class MGAT_dataset(torch.nn.Module):
    ...
    def init_edges(self):
        ...
        self.row_pointers, self.column_index, self.values,self.values_templete=MagicsphereBlock_cmake.blockProcess8_16_tf32(self.row_pointers, self.column_index)
        result = self.row_pointers[2::2] - self.row_pointers[:-2:2]
        self.max=max(result)
        # _, _ ,self.values_templete = MagicsphereBlock_cmake.blockProcess_output_8_4(row, col)
    ...
    def init_embedding(self):
        ...
        '''
        self.x = torch.from_numpy(self.graph['features']).to(dtype=torch.float32)
