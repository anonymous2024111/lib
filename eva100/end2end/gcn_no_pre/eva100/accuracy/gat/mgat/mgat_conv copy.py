...
class MGATFunction(torch.autograd.Function):
    ...
    def forward(ctx, X_prime, a0, a1, inputInfo):
        ...
        att= MagicsphereGAT_cmake.forward(X_prime.size(1), inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, inputInfo.values, X_prime, a0, a1,inputInfo.max)
        #过滤掉0元素
    ...
    def backward(ctx, att_grad):
        ...
        '''
        #att_grad求a0梯度
        a0_tmp= MagicsphereGCN_cmake.forward(inputInfo.row_pointers, inputInfo.column_index, att_grad, X_prime, inputInfo.num_nodes, X_prime.size(1), inputInfo.num_nodes_ori)
        #按列求和，求出a0
        a0_grad = torch.mm(inputInfo.ones.t(), a0_tmp)
        #att_grad求a1梯度
        a1_tmp= MagicsphereGCN_cmake.forward(inputInfo.row_pointers, inputInfo.column_index, att_grad, inputInfo.ones, inputInfo.num_nodes, inputInfo.ones.size(1), inputInfo.num_nodes_ori)
...
class MGATSpmm(torch.autograd.Function):
    ...
    def forward(ctx, att, X_prime, inputInfo):
        ...
        X_prime = MagicsphereGCN_cmake.forward(inputInfo.row_pointers, inputInfo.column_index, att, X_prime, inputInfo.num_nodes, X_prime.size(1), inputInfo.num_nodes_ori)
        return X_prime
    ...
    def backward(ctx, X_prime_grad):
        ...
        #求X_prime_grad
        d_X_prime= MagicsphereGCN_cmake.forward(inputInfo.row_pointers, inputInfo.column_index, att_trans, X_prime_grad, inputInfo.num_nodes, X_prime_grad.size(1), inputInfo.num_nodes_ori)
...
#         # GEMM node update
#         # SpMM: Neighbor AggreAGNNion.
#         att=mGAT.softmax_gat(inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, att, rows_sum, inputInfo.max, inputInfo.num_nodes_ori)    
#         return att
...
#         att = ctx.att
#         d_att=mGAT.softmax_gat(inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, att_grad, rows_sum, inputInfo.max, inputInfo.num_nodes_ori)    
#         d_rows_sum = mGAT.softmax_gat(inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, att_grad, att, inputInfo.max, inputInfo.num_nodes_ori)    
...
class GATConv(torch.nn.Module):
    ...
    def forward(self, X, inputInfo):
        ...
        #temp = att
        att = torch.exp(att)
        rows_sum = MGATSpmm.apply(att, inputInfo.ones, inputInfo)
