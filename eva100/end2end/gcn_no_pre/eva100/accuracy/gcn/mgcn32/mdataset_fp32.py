...
class MGCN_dataset(torch.nn.Module):
    ...
    def init_edges(self):
        ...
        #self-loop
        adj = sp.coo_matrix((np.ones(len(src_li)), self.m_edge_index_new),
        ...
        dd= torch.rsqrt(dd) 
        #bcsr
        self.row_pointers, self.column_index, self.degrees=MagicsphereBlock_cmake.blockProcess8_4(self.row_pointers, self.column_index,dd)
