...
__global__ void generate_edgetocolumn_libra_spmm(int *nodePointer, int *edgelist,
                                      int *edgelist_sort, int *counts, int *edgetocol, int *blockpartition, 
	...
	//vector个数
	blockpartition[winId] = vector_num;
...
void generate_edgetocolumn_cuda_libra_spmm(int *nodePointer, int *edgelist,
                                int *edgelist_sort, int *counts, int *edgetocol, int *blockpartition, 
	...
	//   int block_size1 = 128;
	//   int block_count1 = (window_count + 127) / 128;
  	generate_edgetocolumn_libra_spmm<<<window_count, block_size>>>(
      nodePointer, edgelist, edgelist_sort, counts, edgetocol, blockpartition, 
...
seg_sort_dequ(int *seg, int *edgeLists, int *nodepointer, int *edgetocol,
  ...
  auto values_tensor = torch::zeros({num_edges}, torch::kCUDA).to(torch::kF32);
  ...
  generate_tcoffset_id_atob_cuda(
      ...
      tcblock_offset + 1, tcblocktile_id, values, sparse_AToX_index, max_blocks,
...
seg_sort_dequ_libra_spmm(int *seg, int *edgeLists, int *nodepointer, int *edgetocol,
	...
	//   int max_blocks = (*max_element + blockSize_w) / blockSize_w;
  	...
  	//声明最终的数据结构
	auto values_tensor = torch::zeros({h_vector_num*blockSize_h}, torch::kCUDA).to(torch::kF32);
	//   auto tcblocktile_id_tensor = torch::zeros({num_edges}, options_gpu_unit8);
	//   auto tcblock_offset_tensor = torch::zeros({block_counter + 1}, options_gpu);
	auto sparse_AToX_index_tensor =
		torch::zeros({h_vector_num}, options_gpu);
	//   auto tcblock_offset = tcblock_offset_tensor.data<int>();
	auto sparse_AToX_index = sparse_AToX_index_tensor.data<int>();
	//   auto tcblocktile_id = tcblocktile_id_tensor.data<uint8_t>();
	auto values = values_tensor.data<float>();
...
seg_sort_dequ_libra_sddmm(int *seg, int *edgeLists, int *nodepointer, int *edgetocol,
	...
	thrust::device_ptr<int> Counts = thrust::device_pointer_cast(edgeLists);
	thrust::device_vector<int> deviceCounts(Counts, Counts + num_edges);
	generate_edgetocolumn_cuda_libra_sddmm(
		nodepointer, edgeLists, thrust::raw_pointer_cast(&deviceEL[0]), thrust::raw_pointer_cast(&deviceCounts[0]),
		edgetocol,
		blockpartition, block_num, vector_num, blockSize_h, blockSize_w, num_nodes);
