...
std::map<int, int> reorder(adjacency_list adj, auto permNew) {
...
std::vector<torch::Tensor> rabbit_reorder(
    torch::Tensor in_edge_index, int n
) {
  int src, dst;
  ...
  auto permNew = torch::zeros({n},torch::kInt32);
  auto permNew1 = permNew.accessor<int, 1>();
  auto adj = read_graph_from_edges(edges);
  ...
  auto mapping = reorder(std::move(adj), permNew1);
